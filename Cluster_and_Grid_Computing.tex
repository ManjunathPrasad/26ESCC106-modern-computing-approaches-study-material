\begin{center}
  {\Large\bfseries Topic in Module 1 --- Cluster and Grid Computing}
\end{center}
...
grids -- are the subject of the topic in the module. They are important in their own
way, and they are also the foundation on which cloud computing, the next
...
presented by Yeo and Buyya emph{et.al}~\rc{ref:yeo}. It is read from the
bottom up. The
...
A grid can be described from two viewpoints: how the participating sites are
arranged, and how the grid software is organised. Figure~\ref{fig:grid} shows
the first. Each site --- a university cluster, a research laboratory's
supercomputer, a data archive, a scientific instrument, or a pool of
contributed desktop PCs --- remains under its owner's control and keeps its
own local scheduler and policy. A layer of grid middleware at each site acts
as a gateway, translating requests from the grid into local actions that the
owner has agreed to permit. The sites are linked only by the wide-area
network, and together they form the virtual organisation described above.

A user does not approach the sites one by one. They authenticate once, using
a digital certificate, and submit work to a grid portal or resource broker.
The broker asks the information service which resources are currently
available and suitable, and the security service establishes that the user
is trusted at each site chosen, so that one login is accepted everywhere
(single sign-on). The broker then dispatches the job, and the necessary data,
across the network to those sites.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item What is a Beowulf cluster? Explain the economic argument that
        motivated it.
  ...
  \item Define grid computing and explain the origin of the term.
  ...
  \item What is volunteer computing? Explain what kind of problem it is
        suited to and why.
  ...
  \item Explain the layered grid architecture with a diagram, stating the
...
\begin{enumerate}[label={[\arabic*]},ref=\arabic*,leftmargin=2.2em]
  ...
  \item\label{ref:anatomy} I.~Foster, C.~Kesselman and S.~Tuecke, ``The
        ...
        the layered architecture in Figure~\ref{fig:layers}.
